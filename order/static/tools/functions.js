...
'            <input type="text" name="cast[]" id="cast" class="form-control" placeholder="Firstname, Lastname">'+
...
$("input[name='cast[]']").change(function () {
      $.ajax({
           type: "POST",
           url: "dvdRegistration.html",
           // data: $(this).parent().serialize(), // changed
           // data: $('#cast').val(),
            data: $("input[name='cast[]']").serialize(),
            contentType: false,
...
$("input[name='imageLink[]']").change(function () {
      $.ajax({
           type: "POST",
           url: "index",
           // data: $(this).parent().serialize(), // changed
           // data: $('#cast').val(),
            data: $("input[name='imageLink[]']").serialize(),
            contentType: false,
           success: function(data) {
               console.log(data); // show response from the php script.
...
$("input[name='fileses']").change(function () {
      $.ajax({
           type: "POST",
           url: "index",
           // data: $(this).parent().serialize(), // changed
           // data: $('#cast').val(),
            data: $("input[name='fileses']").serialize(),
            contentType: false,
           success: function(data) {
               console.log(data); // show response from the php script.
...
$("input[name='deleteInfo']").change(function () {
      $.ajax({
           type: "POST",
           url: "index",
           // data: $(this).parent().serialize(), // changed
           // data: $('#cast').val(),
            data: $("input[name='deleteInfo']").serialize(),
            contentType: false,
           success: function(data) {
               console.log(data); // show response from the php script.
...
$(document).ready(function() {
  ...
  if (window.File && window.FileList && window.FileReader) {
    ...
    var img = values[c];
    $("#files-"+img).on("change", function(e) {
      var ee = e;
   var fff = e.target.id;
        ...
        fileReader.onload = (function(e) {
          ...
          $("<span class=\"pip\">" +
            ...
            "<br/><span class=\"removes\"> <svg width=\"2em\" height=\"2em\" viewbox=\"0 0 16 16\" class=\"bi bi-trash text-danger\" fill=\"currentColor\" xmlns=\"http://www.w3.org/2000/svg\">"+
                                           ...
                                           "</svg></span>" +
            "</span>").insertAfter("#"+fff);   
          // console.log("target by"+fff); console.log("sa con "+"#files-"+img);
